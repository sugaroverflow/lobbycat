...
import { runSafetyFrameworksPipeline } from "@/lib/safety-frameworks";

export const dynamic = "force-dynamic";
export const maxDuration = 120;

/**
 * GET /api/cron/safety-frameworks
 *
 * Re-ingests `src/db/safety-frameworks-seed.json` into `safety_frameworks`.
 * v0.6 evidence pipeline #5 — Step 11.
 *
...
export async function GET(request: Request) {
  ...
  try {
    const result = await runSafetyFrameworksPipeline({ dryRun, onlySlugs });
