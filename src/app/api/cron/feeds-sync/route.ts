...
import { runFeedsSync } from "@/lib/feeds-sync";
 ...
 *
 * Query params:
 *   ?slugs=anthropic-london,wayve   — restrict to specific slugs
 *   ?dryRun=1                       — read+plan but do not write
 */
export async function GET(request: Request) {
  ...
  try {
    const result = await runFeedsSync({ onlySlugs, dryRun });
