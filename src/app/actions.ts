...
/**
 * Generate (or refresh) a fit note for a company. Thin wrapper around the
 * shared `generateFitNoteForCompany` helper (`src/lib/fit-notes/generate.ts`)
 * so the same generation path runs whether the trigger is a user click,
 * the v0.7.2 one-off backfill script, or Glyphie's nightly cron.
 */
export async function generateFitNote(companyId: number) {
  const { generateFitNoteForCompany } = await import("@/lib/fit-notes/generate");
  const result = await generateFitNoteForCompany(companyId, { source: "manual" });
  if (result.status === "failed") {
    throw new Error(`fit-note generation failed: ${result.error}`);
  }
  revalidatePath(`/companies/[slug]`, "page");
}
