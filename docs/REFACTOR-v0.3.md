...
| 6 | The Map | in progress | 75m | 2D scatter, axes = any two scale-kind frames, hover-cards, click-through. 6a ✅ route split (`/` = map, `/companies` = tier list) + axis pickers + SVG scatter + hover-card v1. 6b ✅ richer hover-card (top-4 tag pills + fit-note preview) + click-to-pin with × close. 6c pending: filter chips + axis-swap button. |
