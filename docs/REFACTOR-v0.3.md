...
| 6 | The Map | in progress | 75m | 2D scatter, axes = any two scale-kind frames, hover-cards, click-through. 6a ✅ route split (`/` = map, `/companies` = tier list) + axis pickers + SVG scatter + hover-card v1. 6b pending: richer hover-card (tags, fit-note preview) + sticky pin. 6c pending: filter chips + axis-swap button. |
